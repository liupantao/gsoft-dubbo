 ...
 * 
 * 修改监控发送代码，将 DubboService 接口的通用调用方法转换为实际调用的service.method
 * 
...
public class MonitorFilter implements Filter {
	...
	private void collect(Invoker<?> invoker, Invocation invocation, Result result, RpcContext context, long start, boolean error) {
        try {
            ...
            if (service.equals(DubboService.class.getName()) && method.equals("exchange")) {
				Object params = invocation.getArguments()[0];
				if (params != null && params instanceof Map) {
					Map<String, String> headers = (Map) params;
